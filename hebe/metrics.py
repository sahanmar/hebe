...
def calculate_roc_auc(
    predictions: np.ndarray | torch.Tensor, ground_truth: np.ndarray | torch.Tensor
) -> float:
    """
    Calculate the Area Under the ROC Curve (ROC AUC) for binary classification.

    Args:
        predictions (torch.Tensor): Predicted probabilities or scores for the positive class.
